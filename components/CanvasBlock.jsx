...
function toEmbed(url){
  ...
  try{
    ...
    if(host.includes('youtube.com')){ const v = u.searchParams.get('v'); if(v) return `https://www.youtube.com/embed/${v}` }
    if(host.includes('youtu.be')){ const id = u.pathname.slice(1); if(id) return `https://www.youtube.com/embed/${id}` }
    if(host.includes('vimeo.com')){ const id = u.pathname.split('/').filter(Boolean).pop(); if(id) return `https://player.vimeo.com/video/${id}` }
  }catch(e){}
  return url
}
...
export default function CanvasBlock({ block, editMode, onChange, onUpdatePos, onSelect, selected }){
  ...
  function content(){
    ...
    if(type==='video'){
      ...
      return <div style={{ width:'100%', height:'100%', position:'relative', paddingTop:'56.25%' }}>
        <iframe src={embed} style={{ position:'absolute', top:0, left:0, width:'100%', height:'100%' }} frameBorder='0' allowFullScreen />
      </div>
    }
  ...
  return (
    <Rnd
      ...
      <div className='card' style={styleObj}>
        {type==='text' ? (
          <div contentEditable={editMode} suppressContentEditableWarning onBlur={e=> onChange(id, e.target.innerHTML)} style={{ minWidth:50 }}>
